...
use Cariboo\Payment\SipsBundle\Client\Authentication\AuthenticationStrategyInterface;
...
class Client
{
    ...
    public function requestDoExpressCheckoutPayment($token, $amount, $paymentAction, $payerId, array $optionalParameters = array())
    {
        ...
        if ($this->isDigital)
        {
            $params['PAYMENTREQUEST_0_ITEMAMT'] = $this->convertAmountToPaypalFormat($amount);
            $params['L_PAYMENTREQUEST_0_ITEMCATEGORY0'] = 'Digital';
            $params['L_PAYMENTREQUEST_0_NAME0'] = 'Abonnement Bébé Nounou';
            $params['L_PAYMENTREQUEST_0_QTY0'] = 1;
            $params['L_PAYMENTREQUEST_0_AMT0'] = $this->convertAmountToPaypalFormat($amount);
        }
        return $this->sendApiRequest(array_merge($optionalParameters, $params));
    }
